...
| 问题 | 严重程度 | 表现 |
|------|:--:|------|
| 资源就绪检查不充分 | 🔴 高 | `networkidle0` 可能提前触发，导致截图时图片未加载完成 |
| Google Fonts 加载延迟 | 🔴 高 | Noto Serif SC 在线字体可能未就绪即截图，出现缺字 |
| 浏览器单例无重连机制 | 🟡 中 | 浏览器崩溃后所有后续请求失败 |
| 无错误诊断抓取 | 🟡 中 | 渲染失败时缺少页面状态快照，难以排查 |
| HTML 模板耦合在 TS 中 | 🟡 中 | 300+ 行模板字符串，CSS 无语法高亮，难以维护 |
| 无并发渲染能力 | 🟢 低 | 单 Page 串行，高并发时响应慢 |
| 设计令牌硬编码 | 🟢 低 | 颜色/字号/间距写死在模板字符串中，无法切换主题 |
| 缺乏性能监控 | 🟢 低 | 无渲染耗时、缓存命中率等指标 |
...
```
Week 1-2  ████████  Phase 1: 稳定性修复
          ├── 1.1 增强资源就绪检查
          ├── 1.2 字体本地化
          ├── 1.3 浏览器重连机制
          └── 1.4 错误诊断抓取

Week 3-4  ████████  Phase 2: 架构优化
          ├── 2.1 HTML 模板外挂化
          ├── 2.2 设计令牌系统
          └── 2.3 浏览器 Page 池化
